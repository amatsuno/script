...
--roll
    ...
    local roll_enhance = set_combine(roll, {
        right_ring='ルザフリング',
    })
...
--神聖
    ...
    sets.precast['ロール'] = roll
    sets.precast['ロール範囲'] = roll_enhance
    sets.precast.FC = {}
    ...
    --マクロブック、セット変更
    send_command('input /macro book 9;wait .2;input /macro set 1')
...
function precast(spell)
    ...
    elseif spell.type == 'CorsairRoll' then
        if roll_list:contains(spell.name) then
            set_equip = sets.precast['ロール範囲']
        else
            set_equip = sets.precast['ロール']
        end
...
function self_command(command)
    ...
    if #args >= 1 then
        ...
        elseif args[1] == 'si' then
            my_send_command('mogmaster si cor')
        end
    end
    if #args >= 2 then
